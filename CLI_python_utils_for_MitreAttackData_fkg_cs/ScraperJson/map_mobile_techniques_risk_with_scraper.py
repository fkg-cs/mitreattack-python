...
def get_technique_risk_scores(techniqueKeyWords):
    ...
    techniqueKeyWords = sanitize_keyWords(
        techniqueKeyWords)  # elaboro le keyword e tolgo eventuali caratteri problematici

    print(f"--> RETRIVING INFORMATION OF All CVEs FROM dir  <--\n")
    list_all_cves_datas = get_list_of_CVEs_from_dir(
        r"../../Janus/json/json_CVE")  # ottengo lista con i dati di TUTTI i cve presenti nella cartella json
        ...
        if cve_data.has_metrics():  # solo se ha le metriche che mi servono controllo la descrizione
            ...
            for word in techniqueKeyWords.split():  # controllo se la descrizione della tecnica sudivisa in parole chiave ha corrispondenza con la descrizione della cve
                if word not in commonWords and (word.lower() in cve_data_description.lower().split()):
                    if not is_cve_id_present(cve_data.get_cveId(),
                                             list_cves_matching_keyword):  # faccio append solo se non l'ho gia inserito nella lista
                        ...
                        print(
                            f"--> CVE ID with metrics and keywords ('{techniqueKeyWords}')->('{word}')  in description: {cve_data.get_cveId()} ")
                        ...
                        print(
                            f"                ATTACK CONFIDENTIALITY IMPACT: {cve_data.get_cvss_confidentialityImpact()} ")
    ...
    print(
        f"---REPORT: media delle confidentiality impact: {calculate_average_confidentialityImpact(list_cves_matching_keyword)}")
    print(f"---REPORT: media delle integrity impact: {calculate_average_integrityImpact(list_cves_matching_keyword)}")
    print(
        f"---REPORT: media delle avalabilityty impact: {calculate_average_availabilityImpact(list_cves_matching_keyword)}")
...
def get_mobile_techniques_risk_scores():
    ...
    print("----------Scraping CVSS3.1 metrics for all Mobile techniques------------")
    ...
    print("------------------------STATISTICS OF MOBILE SCORES-------------------------------")
    print("MINIMUM SCORE: ", min_score)
    print("MAXIMUM SCORE: ", max_score)
    print("AVERAGE SCORE: ", average_score)
    print("NUMBER OF TECHNIQUES MAPPED: ",len(techniques))

    mapping= dict(sorted(mapping.items(), key=lambda x: x[1], reverse=True)[:10])#ordino mapping in ordine descrescente per valore basescore e prendo i top 10
    print("TOP 10 SCORES in MOBILE:")
    i=0
    ...
    plt.title('Grafico dei risk scores calcolati per le tecniche in MOBILE')
...
if __name__ == "__main__":
    ...
    get_mobile_techniques_risk_scores()