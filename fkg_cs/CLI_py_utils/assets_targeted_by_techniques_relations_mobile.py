...
def main():
    ...
    mitre_attack_data = MitreAttackData("../json/json_matrix/mobile-attack.json")
    tactics = mitre_attack_data.get_tactics(remove_revoked_deprecated=True)

    n_tactics = len(tactics)

    print("-----TREE OF TACTICS AND TECHNIQUES IN MOBILE ATT&CK AND TARGETED ASSETS RELATION-----\n")
    ...
    for tactic in tactics:
        ...
        techniques = mitre_attack_data.get_techniques_by_tactic(
            tactic.x_mitre_shortname, "mobile-attack", remove_revoked_deprecated=True)
        ...
        for technique in techniques:
            ...
            for ref in external_references:
                if (
                        ref.get(
                            "source_name") == 'mitre-attack'):  # assegno solo se mi trovo nella external reference di mitre attack
            ...
            print(
                f"   {i}.{j}) {technique.name} (ATT&CK ID: {technique_external_id}) targets ")  #: {technique.description}#stampa descrizione ma riduce legibilità

            match len(assets_targeted):
                case 0:
                    print("           no asset.")
                case 1:
                    print("         asset:")
                case _:
                    print(f"         {len(assets_targeted)} assets:")

            i_assets = 0
            for asset_targeted in assets_targeted:
                i_assets = i_assets + 1
                asset = asset_targeted["object"]
                print(f"        {i_assets}) {asset.name} (ASSET ATT&CK ID: {mitre_attack_data.get_attack_id(asset.id)}): {asset.description}")
